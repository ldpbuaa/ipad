...
class CMISynthesizer(BaseSynthesis):
    def __init__(self, teacher, student, generator, nz, img_size, train_class_counts, num_classes,
    ...
    def synthesize(self,  epoch, step, targets=None):
        ...
        loss_bn_total, loss_oh_total, loss_adv_total, loss_act_total, loss_balance_total, loss_contra_total = 0, 0, 0, 0, 0, 0
        loss_act, loss_balance = torch.tensor(0), torch.tensor(0)
        for it in range(self.iterations):
            ...
            loss_contra_total += loss_cr.detach().cpu()

        self.student.train()
        # save best inputs and reset data iter
        # only update data pool at the begining of each epoch
        if step==0:
        ...
        return {"synthetic": best_inputs,
                ...
                'loss_contra_total':loss_contra_total}
